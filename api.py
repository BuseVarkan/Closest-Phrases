...
The top2 endpoint accepts a list of phrases
and returns the top 2 closest phrases.

Calculating the top 2 closest phrases is done by
calculating the cosine distance between
encodings of all pairs of phrases and
returning the pair with the smallest distance.
...
def check_phrases(phrases: list[str]) -> HTTPException:
  ...
  for phrase in phrases:
    if not isinstance(phrase, str):
      raise HTTPException(status_code=400,
                detail="Error: Phrase is not a string.")
    if len(phrase) == 0:
...
def calculate_embeddings(phrases: list[str]) -> list[list[float]]:
  """
  Calculate the embeddings for a list of phrases
  using the pre-trained sentence embedding model.
...
def calculate_cosine_distance(embedding1: list[float],
